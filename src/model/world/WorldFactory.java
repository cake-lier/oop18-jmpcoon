 ...
 * {@link World} per game and the {@link controller.game.GameController} can't use all of {@link World} methods, some are 
 * reserved for {@link model.entities.Entity}s exclusive use only. This factory is intended for enforcing such rules. Because 
 * multiple games can be created, multiple {@link World}s can be created, but this should be done from different 
 * {@link controller.game.GameController}s. For meeting those two different needs, for each factory, only one {@link World} can 
 * be created, but at the same time multiple factories can be created. It will be {@link controller.game.GameController} 
