...
public class CollisionRules extends CollisionAdapter implements Serializable {
    ...
    /**
     * {@inheritDoc}
     * The only collisions that should be managed are the ones between the {@link model.entities.Player} and the other
     * {@link model.entities.Entity}s. If the {@link model.entities.Player} collides with a {@link model.entities.PowerUp},
     * this last one should be destroyed and the {@link model.entities.Player} should acquire its power, unless the
     * {@link model.entities.PowerUp} is the goal and as such the game should end. After this, the {@link NotifiableWorld}
     * should be notified that a {@link model.entities.PowerUp} was acquired.
     * If the {@link model.entities.Player} collides with an enemy, such as a {@link model.entities.WalkingEnemy} or a
     * {@link model.entities.RollingEnemy}, if it's invincible or it collides with it on the top of the enemy, the enemy
     * should die and the {@link NotifiableWorld} gets informed of it. If the player is invulnerable the collision simply
     * is ignored, as never happened in the first place, otherwise the {@link model.entities.Player} gets hit and, if it
     * has no lives left, it should die, and the {@link NotifiableWorld} get notified of this.
     * If the {@link model.entities.Player} collides with a {@link model.entities.Platform} on its top and does this
     * while it's climbing up and is at the top of the {@link model.entities.Ladder} or is climbing down and is at the
     * bottom of it, this means the {@link model.entities.Player} reached an end of the {@link model.entities.Ladder}
    ...
    private void processPowerUp(final PlayerPhysicalBody playerPhysicalBody, final Body powerUpBody) {
        ...
        final PowerUpType type = this.physicalWorld.getPowerUpTypeFromBody(powerUpBody);
